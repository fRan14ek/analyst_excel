...
class ExcelProcessorTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.base_dir.mkdir()
        self.previous_base_dir = excel_processor.BASE_DIR
        excel_processor.BASE_DIR = self.base_dir
        self.workbook_name = "sample.xlsx"
        self.workbook_path = self.base_dir / self.workbook_name
        _write_sample_workbook(self.workbook_path)

    def tearDown(self) -> None:
        excel_processor.BASE_DIR = self.previous_base_dir
        self.temp_dir.cleanup()

    def test_process_workbook_creates_expected_artifacts(self) -> None:
        output_dir = Path(self.temp_dir.name) / "output"
        dictionary_path = excel_processor.process_workbook(self.workbook_name, output_dir=output_dir)

        self.assertTrue(dictionary_path.exists(), "JSON-словарь должен быть создан")
